...
const kgToLbsScene = new Scenes.BaseScene("kgToLbs");
...
const lbsToKgScene = new Scenes.BaseScene("lbsToKg");
...
lbsToKgScene.on(message("text"), (ctx) => {
    ...
    const kg = lb / 2.20462;
    ctx.reply(`${lb} lbs = ${kg.toFixed(2)} kg\n\n` + `您可以选择继续还是退出:`, getContinueKeyboard());
});
...
bot.command("start", (ctx) => {
    ctx.reply("你好! 欢迎使用杠铃重量转换器!", Markup.keyboard([["Kg ➡️ Lbs", "Lbs ➡️ Kg"], ["Setting"]]).resize());
});
